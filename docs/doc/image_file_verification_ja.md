...
## 3. フィルタの多段接続（パイプライン検証）

テストは 1 シミュレーションにつき 1 スロットを実行するが、多段パイプラインは
**実行のカスケード**で検証できる: 各段のキャプチャ `output.ppm` を次段の `-Image` 入力に
渡す。各段は引き続きゴールデンとピクセル厳密に比較される。例 — 定番の
...
設計選択）であり、パターン右側のバー（マゼンタ / 赤 / 青 / 黒）はすべて G=0 である。
ゴールデンモデルはこれを正確に再現するため、このチェーンはハードウェアが実際に計算する
内容を検証する — 第 2〜3 段が緑輝度の変化する箇所（対角線とグラデーション/黒の境界）
だけでエッジを検出しているのはそのためである。
